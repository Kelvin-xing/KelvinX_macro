...
par.labda2     = par.labda1*par.b*par.C_ss^(-par.nu);
...
switch accuracylinear
    case 'yes'
        ...
        %generate C(t), K(t), & L(t):
            ...
            L_simulB(t)  = (par.b*par.labda1*CondExp*K_simulB(t)^par.alpha/par.labda2)^(1/(par.chi+1-par.beta));
...
%        L_simul(t) = exp(log(par.L_ss) + l_tilde);
        ...
        %disp([C_simul(t) L_simul(t) K_simul(t)])
        %pause
    end
...
switch accuracyprojection
    case 'yes'
        ...
        for t = 2:T            
            ...
            C_t       = (mu_t)^(-1/par.nu);
            L_t       = (par.b*par.labda1*mu_t*K_simulA(t)^par.alpha/par.labda2)^(1/(par.chi+1-par.beta));
            ...
            L_simulA(t) = (par.b*par.labda1*C_simulA(t)^(-par.nu)*K_simulA(t)^par.alpha/par.labda2)^(1/(par.chi+1-par.beta));
