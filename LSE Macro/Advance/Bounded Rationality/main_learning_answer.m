...
%% Solving for the REE
%
% the REE solution for p(t) is of the form B0_j+B1_j*z(t) 
% where j indicates the regime
% 
% the following lines set up the system to solve for Bi_lo & Bi_hi
%
% Bi = XX1 + XX2*Bi
%
% the rows of Bi correspond to the low and the high regime
% the first column of Bi correspond to the constant and the second to the z variable
%
% Bi = B0_ree + B1_ree*z
%
% first solve for the constant

XX1 =  par.beta ...
    * [par.rho*par.mu_lo + (1-par.rho)*par.mu_hi;...
       par.rho*par.mu_hi + (1-par.rho)*par.mu_lo]   ;
XX2 =    par.beta ...
    * [  par.rho, 1-par.rho; ...
       1-par.rho,   par.rho]    ;

B0_ree = (eye(2)-XX2)\XX1;

% now solve for the slope

XX1 =  par.beta*par.rho_z*ones(2,1);
XX2 =  par.beta*par.rho_z*[par.rho,1-par.rho;1-par.rho,par.rho];
B1_ree  = (eye(2)-XX2)\XX1;
...
for t = 2:par.T
    ...
    res_lo         = dividend(t) - par.mu_lo - par.rho_z*z(t-1);
    res_hi         = dividend(t) - par.mu_hi - par.rho_z*z(t-1);
    
    % step II: update beliefs on current state and 
    % implied probs for next period's state
    
    xi_tt(t,:)     = xi_tnextt(t-1,:)'.*[normpdf(res_lo,0,par.sigma); normpdf(res_hi,0,par.sigma)];
    xi_tt(t,:)     = xi_tt(t,:)./sum(xi_tt(t,:));
    ...
    price_bay(t)   = xi_tt(t,1)*(B0_ree(1) + B1_ree(1)*z(t)) ...
                   + xi_tt(t,2)*(B0_ree(2) + B1_ree(2)*z(t))     ;
    ...
    if t > par.T1       
            ...
            Y        = price_amn(t-1);
            RR       = RR     + (1/par.T1)*(X'*X-RR);
            beta_p   = beta_p + (1/par.T1)*(RR\X')*(Y-X*beta_p);
            
            % calculate current price if next-period's price is next-period's price is believed to be determined by this process
        
            % no change in regime            
            ind_mu_next = ind_mu(t);
            IND_mu_next = 1.5+0.5*ind_mu_next;
            RHS_next_1 = par.beta*([1 ind_mu_next par.rho_z*z(t) ind_mu_next*par.rho_z*z(t)]*beta_p + par.mu(IND_mu_next) + par.rho_z*z(t));
            %    change in regime            
            ind_mu_next = -ind_mu(t);
            IND_mu_next = 1.5+0.5*ind_mu_next;
            RHS_next_2 = par.beta*([1 ind_mu_next par.rho_z*z(t) ind_mu_next*par.rho_z*z(t)]*beta_p + par.mu(IND_mu_next) + par.rho_z*z(t));
...
figure(2)
plot([price_ree(par.T1+1:1400) price_bay(par.T1+1:1400)])
figure(3)
plot([price_ree(par.T1+1:1400) price_lsq(par.T1+1:1400)])
