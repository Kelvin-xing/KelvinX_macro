...
thetahat    = Xp'\yp;            %this results in standard OLS estimate
err         = yp-Xp'*thetahat;
Sigma       = err'*err/np;
varthetahat = Sigma*inv(Xp*Xp');             %#ok
%without the "%ok" added Matlab would put a warning indicating that inv(.)
%may be slow and inaccurate. If you are aware of such warnings and would
%like to suppress them, then simply add "%ok"
...
Qbar = T0Q*gam2*varthetahat;        %scale matrix inverse-Wishart prior Q
...
Rbar = T0R*Sigma;               %scale matrix inverse-Wishart prior R
...
for i = 1:g
    ...
    %Sample TH step 1 of the Gibbs sampler
    TH(:,1,i) = THbar;
    ...
    Qtemp = v*v'+Qbar; %calculate scalings matrix posterior
    Qtemp = (Qtemp+Qtemp')/2;  %this should do nothing since Qtemp should be symmetric (but could be good to add in case minor inaccuracies introduce asymmetry)
    Q1(:,:,i+1) = iwishrnd(Qtemp,T+T0Q); %iwishrnd(A,I) draws from inverted Wishart with scalings matrix A and I dgf
    ...
    Rtemp = u'*u+Rbar;  %calculate scalings matrix posterior
    Rtemp = (Rtemp+Rtemp')/2; % see comment aboves
