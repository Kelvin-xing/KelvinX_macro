...
for t = 1:T
    Xtemp       = X(:,t);
    Po(:,:,t)   = Pe(:,:,t)+Q;
    K           = Po(:,:,t)*Xtemp/(Xtemp'*Po(:,:,t)*Xtemp+R);
...
%Draw from TH(T|T)
TH(:,T+1) = TH(:,T+1)+chol(Pe(:,:,T+1))'*draw(:,T+1);
...
%
for tt = T:-1:1
    mean     = TH(:,tt)+(Pe(:,:,tt)/Po(:,:,tt))*(TH(:,tt+1)-TH(:,tt));
    variance = Pe(:,:,tt)-(Pe(:,:,tt)/Po(:,:,tt))*Pe(:,:,tt);
    TH(:,tt) = mean +...
              chol(variance)'*draw(:,tt);
end
