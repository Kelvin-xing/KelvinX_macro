...
[diseq_r_l,ks_l] = diseq_full(r_l,par,z,shocks);
[diseq_r_u,ks_u] = diseq_full(r_u,par,z,shocks);
[diseq_r_x,ks_x] = diseq_full(r_x,par,z,shocks);
...
while err > par.tol
    ...
    elseif test < 0
        r_u = r_x;        % root is below r_x
        diseq_r_u = diseq_r_x;
    else
        r_l = r_x;        % root is above r_x
        diseq_r_l = diseq_r_x;
    end
...
disp(' ')
disp('Partial equilibrium capital supply and \% change')
disp([ks_partial(1), 100*(ks_partial(2)-ks_partial(1))/ks_partial(1)])
disp('General equilibrium capital supply and \% change')
