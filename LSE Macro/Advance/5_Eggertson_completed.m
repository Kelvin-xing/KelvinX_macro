...
while metric>1e-13
    ...
    E_t1 = inv(B_t+C_tn*F_n+C_tt*F_t)*(-(C_tn*E_n+C_tt*E_t+D_t));

    F_n1 = inv(B_n+C_nn*F_n+C_nt*F_t)*(-A_n);
    F_t1 = inv(B_t+C_tn*F_n+C_tt*F_t)*(-A_t);
...
if i<10
        X10(:,i+1) = E_t+F_t*X10(:,i);
        X10g(:,i+1) = E_t+F_t*X10g(:,i);
    else
        X10(:,i+1) = E_n+F_n*X10(:,i);
        X10g(:,i+1) = E_n+F_n*X10g(:,i);
    end
    if i<20
        X20(:,i+1) = E_t+F_t*X20(:,i);
        X20g(:,i+1) = E_t+F_t*X20g(:,i);
    else
        X20(:,i+1) = E_n+F_n*X20(:,i);
        X20g(:,i+1) = E_n+F_n*X20g(:,i);
    end
    if i<30
        X30(:,i+1) = E_t+F_t*X30(:,i);
        X30g(:,i+1) = E_t+F_t*X30g(:,i);
    else
        X30(:,i+1) = E_n+F_n*X30(:,i);
        X30g(:,i+1) = E_n+F_n*X30g(:,i);
    end
    if i<40
        X40(:,i+1) = E_t+F_t*X40(:,i);
        X40g(:,i+1) = E_t+F_t*X40g(:,i);
    else
        X40(:,i+1) = E_n+F_n*X40(:,i);
        X40g(:,i+1) = E_n+F_n*X40g(:,i);
    end
