...
% Now repeat the exercise with the model with investment technology shocks

dynare RBCInvestment.mod noclearall

load dynarerocks

% The loop that generates artificial data series, this time for the model
% with investment technology shocks 
for i = 2:T
    S = [1, k(i-1)-k_ss, z(i-1)-z_ss, shocks(i-1)];
...
%Store the computed series in the second column:
outstore(:,2) = y;
constore(:,2) = c;
invstore(:,2) = inv;
