...
% To generate the artificial data, we will start at the steady state. Note
% that for the second-order approximation, starting values are important!
c(1,1) = c_ss;
k(1,1) = k_ss;
z(1,1) = z_ss;
...
for i = 2:T
    ...
    S = [1,...
         ...
         shocks(i),...
         (k(i-1)-k_ss(1,1))^2,...
         (z(i-1)-z_ss(1,1))*(k(i-1)-k_ss(1,1)),...
         (z(i-1)-z_ss(1,1))^2,...
         shocks(i)^2,...
         (k(i-1)-k_ss(1,1))*shocks(i),...
         (z(i-1)-z_ss(1,1))*shocks(i)];
...
dynare RBClogs1st.mod noclearall
% The command noclearall means that variables defined earlier (like T) are not deleted 
...
dynare RBClogs2nd.mod noclearall
% The command noclearall means that variables defined earlier (like T) are not deleted 
