...
L = eta^(1/(1-eta))*(W/alpha)^(-gamma)*(alpha^gamma*W^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*(rr+delta).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
K = eta^(1/(1-eta))*((rr+delta)./((1-alpha).*exp(e).^(1-1/gamma))).^(-gamma).*(alpha^gamma*W^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*(rr+delta).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
Y = eta^(eta/(1-eta))*(alpha^gamma*W^(1-gamma) + ((1-alpha)).^gamma.*exp(e).^(gamma-1).*(rr+delta).^(1-gamma)).^(1/(1-gamma)*eta/(eta-1));  
D = Y - W*L - (r+delta)*K;
...
% Compute multipliers on planners constraint to use Ka, La

optset('broyden', 'showiters', 1);

x = broyden('solve_planner', [W;r+delta], Ka, La, e);

Lf = eta^(1/(1-eta))*(x(1)/alpha)^(-gamma)*(alpha^gamma*x(1)^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*x(2).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
Kf = eta^(1/(1-eta))*(x(2)./((1-alpha).*exp(e).^(1-1/gamma))).^(-gamma).*(alpha^gamma*x(1)^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*x(2).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
Yf = eta^(eta/(1-eta))*(alpha^gamma*x(1)^(1-gamma) + ((1-alpha)).^gamma.*exp(e).^(gamma-1).*x(2).^(1-gamma)).^(1/(1-gamma)*eta/(eta-1));  

%Lf = eta^(1/(1-eta))*(W/alpha)^(-gamma)*(alpha^gamma*W^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*(r+delta).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
%Kf = eta^(1/(1-eta))*((r+delta)./((1-alpha).*exp(e).^(1-1/gamma))).^(-gamma).*(alpha^gamma*W^(1-gamma) + (1-alpha)^gamma.*exp(e).^(gamma-1).*(r+delta).^(1-gamma)).^(1/(1-gamma)*(gamma-1/(1-eta)));
%Yf = eta^(eta/(1-eta))*(alpha^gamma*W^(1-gamma) + ((1-alpha)).^gamma.*exp(e).^(gamma-1).*(r+delta).^(1-gamma)).^(1/(1-gamma)*eta/(eta-1));  
