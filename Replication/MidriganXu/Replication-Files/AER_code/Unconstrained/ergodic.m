...
Lp   = eta^(1/(1-eta))*alpha^((1-(1-alpha)*eta)/(1-eta))*(1-alpha)^((1-alpha)*eta/(1-eta))*W^(((1-alpha)*eta-1)/(1-eta)).*(r+delta).^(-(1-alpha)*eta/(1-eta)).*exp(egrid(statep(:,2))+phip);
Kp   = eta^(1/(1-eta))*alpha^(alpha*eta/(1-eta))*(1-alpha)^((1-alpha*eta)/(1-eta))*W^(-alpha*eta/(1-eta)).*(r+delta).^((alpha*eta-1)/(1-eta)).*exp(egrid(statep(:,2))+phip);
Yp   = exp(egrid(statep(:,2))+phip).^(1-eta).*(Lp.^alpha.*Kp.^(1-alpha)).^eta;
Dp   = Yp - W.*Lp - (r+delta).*Kp;  
Cp   = (1-xai)*Dp + (1+r)*statep(:,1) - xp; 

% Unproductive Sector

Lu   = eta^(1/(1-eta))*alpha^((1-(1-alpha)*eta)/(1-eta))*(1-alpha)^((1-alpha)*eta/(1-eta))*W^(((1-alpha)*eta-1)/(1-eta)).*(r+delta).^(-(1-alpha)*eta/(1-eta)).*exp(egrid(stateu(:,2))+phiu);
Ku   = eta^(1/(1-eta))*alpha^(alpha*eta/(1-eta))*(1-alpha)^((1-alpha*eta)/(1-eta))*W^(-alpha*eta/(1-eta)).*(r+delta).^((alpha*eta-1)/(1-eta)).*exp(egrid(stateu(:,2))+phiu);
Yu   = exp(egrid(stateu(:,2))+phiu).^(1-eta).*(Lu.^alpha.*Ku.^(1-alpha)).^eta;
Du   = Yu - W.*Lu - (r+delta).*Ku;  
Cu   = (1-xai)*Du + (1+r)*stateu(:,1) - (xu + kappap*(su==1)); 
...
Ypa = np'*Yp;  % aggregate per capita output productive
Kpa = np'*Kp; 
Lpa = np'*Lp;
...
TFPmbest = (np'*exp(egrid(statep(:,2))+phip) + nu'*exp(egrid(stateu(:,2))+phiu)).^(1-eta);
