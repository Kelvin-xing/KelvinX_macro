...
Ya   = Yma + Yta; 
Ca   = Ya + (1 - delta - mu)*Kma - sum(np)*(mu-1)*kappap - sum(nu)*(mu-1)*kappau  - (1 + r - mu)*D - (sum(np) + sum(nu))*F; 
...
fprintf('Fixed cost                    = %9.3f \n',          F*(sum(np) + sum(nu))/(Lta+Lma));
fprintf('K to Y modern                 = %9.3f \n',          Kma/Yma);
fprintf('aggregate Debt to GDP         = %9.3f  \n',         DGDP); 
