...
a0    = nodeunif(Nf, eps^(1/2), 1-eps^(1/2));
%a0    = exp(norminv(a0, 0, sigmaa)); 

%a0    = log(a0.^(-1/sigmaa));                  % Pareto
a0    = a0.^(-1/sigmaa) - 1;

a0    = min(a0, amax);                          % don't extrapolate
a0    = a0(randperm(Nf));
...
fprintf('Output                        = %9.3f \n',          mean(Y(:))/mean(L(:)));
fprintf('Endowment/Output              = %9.3f \n',          mean(a0)*(1-1/mu)/mean(Y(:)));
