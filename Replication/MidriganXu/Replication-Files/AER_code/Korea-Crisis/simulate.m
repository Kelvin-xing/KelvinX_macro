...
T     = 35;        % periods to simulate allocations
Nf    = 500000;    % # producers
...
for t = 1 : Tinit+T
  ...
  typeold    = type;   % save to determine who pays the fixed cost
        ...
        if any(typeold == 0)
          ...
          %save state at the end of Tinit
          
        if t == Tinit

            sim_steady = [type age agem x a];
            save sim_steady sim_steady; 
...
et = e + phiu.*(type==1);   % total efficiency (exogenous and endogenous)
...
fprintf('Debt to Equity                = %9.2f  %9.2f \n',       [sum(Debt(Debt>0 & type>0))./sum(A(type>0)),       DtoE]); 

fprintf('\n');
finvest = mean(type(:)==1)*kappau*(mu-1);
