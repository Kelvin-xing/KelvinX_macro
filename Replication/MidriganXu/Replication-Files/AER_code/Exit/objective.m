...
global rho alpha eta beta delta W r smin smax fspacep fspaceu ...
       ...
       theta mu lbar Pw phip phiu xai cinj fspacei F
...
kappau = x(1);                % fixed cost of joining u

theta  = x(2); 
xai    = x(3); 

rho    = x(4);               % persistence transitory productivity
se     = x(5);
F      = x(6);
...
amin =  Apmin + exp(-7) + .00;   % tricky right at 0, so move a bit to be able to solve it        
...
amin =  Aumin + exp(-7) + .01;          % tricky right at 0, so move a bit to be able to solve it        
...
y(1) = (Lta + Lma + Lfa - Lbar)./Lbar;

simulate;

y(2) = (sdYm -  sdYdata)./sdYdata;
y(3) = (sY - sYdata)./sYdata;
y(4) = (ac1 - ac1Ydata)./ac1Ydata;
y(5) = (ac3 - ac1Ydata)./ac3Ydata;
y(6) = (ac5 - ac1Ydata)./ac5Ydata;
y(7) = (DtoYm - DtoY)./DtoY;
y(8) = (EtoYm - EtoY)./EtoY;
y(9) = (finvest*numel(Y)/sum(Y(agem>=1))*100 - 4.6)/4.6;
y(10) = (fyoung - 0.73)/0.73;
