...
typem = type;      % sector where guy resides

type(type==2) = typem(typem==2).*condmp(typem==2);
type(type==1) = typem(typem==1).*condmu(typem==1);
...
Eq     = 1/(1+r)*theta*xai*(D + pu.*(typem==1) + pp.*(typem==2));
...
fyoung = sum(agem(:)>=1 & agem(:)< 11)/sum(agem(:)>=1);
...
fprintf('Output                        = %9.3f \n',          mean(Y(:))/(mean(L(:)) + mean(type(:)>=1)*F));
fprintf('Consumption                   = %9.3f \n',          (mean(Y(:)) - (delta+mu-1)*mean(K(:)) - (1+r-mu)*Da  - finvest)/(mean(L(:)) + mean(type(:)>=1)*F));
fprintf('Investment                    = %9.3f \n',          ((delta + mu - 1)*mean(K(:)) + finvest)/(mean(L(:)) + mean(type(:)>=1)*F));
...
TFPlossage = mean(exp(e - (1-alpha)*eta/(1-eta)*YKage))^(1-alpha*eta)/...
          mean(exp(e + (alpha*eta-1)/(1-eta)*YKage))^((1-alpha)*eta)/mean(exp(e))^(1-eta);

TFPlossy  = mean(exp(e(age<=5) - (1-alpha)*eta/(1-eta)*YKres(age<=5)))^(1-alpha*eta)/...
          mean(exp(e(age<=5) + (alpha*eta-1)/(1-eta)*YKres(age<=5)))^((1-alpha)*eta)/mean(exp(e(age<=5)))^(1-eta);
 
TFPlosso  = mean(exp(e(age>10) - (1-alpha)*eta/(1-eta)*YKres(age>10)))^(1-alpha*eta)/...
          mean(exp(e(age>10) + (alpha*eta-1)/(1-eta)*YKres(age>10)))^((1-alpha)*eta)/mean(exp(e(age>10)))^(1-eta);
