...
esim     = zeros(Nf,T);       % transitory component
typesim  = zeros(Nf,T);       % 1 = entrepreneur, 0 = worker 
agesim   = zeros(Nf,T);
agemsim  = zeros(Nf,T);

ee   = randn(Nf,1)*se;
enew = randn(Nf,1)*se/sqrt(1-rho^2);
...
for t = 1 : Tinit+T
    ...
    if t==1
        ...
        type    = e >= ebar;   % transit to E == 1
        
    else
      
        ee       = ee(randperm(Nf));
        enew     = enew(randperm(Nf));
        jump     = jump(randperm(Nf));
       
        e        = (rho*e + ee).*(~jump) + enew.*jump; 
        type     = type.*(~jump);

        age      = age.*(~jump) + 1;
        agem     = (agem + 1).*(type==1);
                
        type(type==0) = e(type==0)>=ebar;
...
z = log(z.^(-1/sz));                            % Pareto
%z  = norminv(z,0,sz);                             % normal with s.d. = se
...
L = exp(e + z)/sum(e(type==1) + z(type==1))*Lm;
K = exp(e + z)/sum(e(type==1) + z(type==1))*K;
