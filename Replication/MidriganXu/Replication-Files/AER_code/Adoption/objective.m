...
phip    = phiu + 1/(1-eta)*0.27;  % productivity gap productive

kappau  = x(1);               % fixed cost of joining u
theta   = x(2); 
xai     = x(3); 
rho     = x(4);              % persistence transitory productivity
se      = x(5);              % stand. dev. transitory shocks
W       = x(6);
r       = x(7);

kappap  = exp(phip-phiu)*kappau;  % fixed cost of joining p
...
y(1) = (Kma - (Apa + Aua + Ata + Aw - Eq))./Kma;
y(2) = (Lta + Lma - Lbar)./Lbar;

simulate1;

y(3) = (std(dY(:)) -  sdYdata)./sdYdata;
y(4) = (sY - sYdata)./sYdata;
y(5) = (ac1 - ac1Ydata)./ac1Ydata;
y(6) = (ac3 - ac1Ydata)./ac3Ydata;
y(7) = (ac5 - ac1Ydata)./ac5Ydata;
y(8) = (DtoYm - DtoY)./DtoY;
y(9) = (EtoYm - EtoY)./EtoY;
y(10)= (finvest*numel(Y)/sum(Y(agem>=1))*100 - 4.6)/4.6;
