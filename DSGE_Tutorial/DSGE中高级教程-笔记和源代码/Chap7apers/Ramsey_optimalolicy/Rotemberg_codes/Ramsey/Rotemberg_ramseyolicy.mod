...
// PARAMETERS
parameters nbeta chi beta epsil phi rho nu pietarget;
...
nbeta = beta;
nu=1/(epsil-1);
//nu=0; %tax subsidy does not satisfiy: nu=1/(epsil-1);
chi=1;
pietarget=1.;
...
%steady state labor
hs=((1+nu)*(epsil-1)+phi*(pis-1)*pis*(1-beta)/
(1+(phi/2)*(pis-1)^2))/(epsil*chi);
hs=sqrt(hs);
...
ramsey_policy(order=1,irf=20, planner_discount=0.99);
