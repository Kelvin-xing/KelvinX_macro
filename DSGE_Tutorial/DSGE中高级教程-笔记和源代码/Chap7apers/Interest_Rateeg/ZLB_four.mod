/*
    ...
    %1st version 2014-04-11@ND
    %2nd version 2016-3-26@Shanghai
    %we consider zero inflation steady state for a simple life here
    %This file is written by Xiangyang Li
*/
var c i pi r n w mc a y yf vp pisharp x1 x2 g nex s1 s2 s3 s4;
varexo ea ei eg ex;
...
mcs = (1-phi*beta*pis^epsilon)/(1-phi*beta*pis^(epsilon-1))*pis/pisharps*(epsilon-1)/epsilon;
...
%(16 -19) for interest rate peg model 
i = s1(-1);
s1 = s2(-1);
s2 = s3(-1);
s3 = s4(-1);
s4 = (1-rhoi)*log(is) + rhoi*s4(-1) + (1-rhoi)*(phipi*(pi(+4) - log(pis)) +phiy*(y(4)- yf(4)))+ ei(+4);
...
stoch_simul(order=1,noprint,periods=200) i pi n w mc y r g c nex yf;
