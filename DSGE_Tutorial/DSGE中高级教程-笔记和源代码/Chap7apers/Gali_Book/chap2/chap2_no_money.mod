...
parameters alpha beta rho sigma phi phi_pi ;
parameters Cs ws pis As Ns Rs rs Ys ;
%----------------------------------------------------------------
...
rho=.9;     //autocorrelation technology shock
sigma=0.9;    //log utility   
...
exp(A+(1-alpha)*N)= exp(Y);

//4. Wage rate, eq. (13)
exp(w)=(1-alpha)*exp(A-alpha*N);

//5. Definition of real interest rate
exp(r)=exp(R-pi(+1)); 

//6. Monetary Policy Rule, eq. (22,24)
// the shock eps_m is totally absorbed by inflation;
R=log(1/beta)+phi_pi*(pi)+eps_m;
//R=(1-0.5)*log(1/beta)+0.5*R(-1)+phi_pi*(pi)+eps_m;
...
stoch_simul(order=1) Y C pi R r N;