/*
 * This mod file implements the baseline Classical Monetary Economy model of Jordi Gali(2008): 
...
parameters Cs ws pis As Ns Rs rs Ys ms;
%----------------------------------------------------------------
...
eta = 4;  // interest rate semi-elasticity of real money demand
...
//9. real money demand, here m denotes the real money
m= Y - eta * R;
//exp(m) = exp(Y)/exp(eta*R);
//exp(m) = exp(Y)/exp(R)^eta;
//exp(m) = exp(Y-eta*R);
end;
...
stoch_simul(order=1) Y C pi R r N m;