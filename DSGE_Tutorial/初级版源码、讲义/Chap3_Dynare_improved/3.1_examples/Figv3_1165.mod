%----------------------------------------------------------------
% Experiment  : Permenant increase in tauk at t=10
%----------------------------------------------------------------


// This program replicates figure 11.6.5 from chapter 11 of RMT3 by Ljungqvist and Sargent
...
// g tauc tauk are now columns of ex_. Because of a bad design decision
...
//The jump in the government policy is reflected in ex_(11,3) for instance.
...
//  The following lines produce a tauk sequence with a once and for all jump in tauk at t=10
// we use shocks to undo that for the first 9 periods and leave tauk at
...
c0=c(1);
k0 = k(1);
// tauk is in ex_(:,3) since it is stored in alphabetical order
tauk0 = ex_(1,3)
...
subplot(2,3,4)
plot(x,[nq0*ones(N,1)],'--k', x,nq(1:40),'k','LineWidth',1.5)
title('\eta','Fontsize',12)
set(gca,'Fontsize',12)

// subplot for the experiment proposed
subplot(2,3,5)
plot([0:9]',ex_(1:10,3),'k','LineWidth',1.5);
hold on;
plot([10:N-1]',ex_(11:N,3),'k','LineWidth',1.5);
hold on;
plot(x,[tauk0*ones(N,1)],'--k','LineWidth',1.5)
axis([0 40 -.1 .5])
title('\tau_k','Fontsize',12)
set(gca,'Fontsize',12)

print -depsc fig_tk.eps
