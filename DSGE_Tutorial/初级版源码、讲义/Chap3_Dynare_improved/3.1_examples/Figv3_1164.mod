%----------------------------------------------------------------
% Experiment  : Permenant increase in tauc at t=10
%----------------------------------------------------------------

// This program replicates figure 11.6.4 from chapter 11 of RMT3 by Ljungqvist and Sargent
...
//The jump in the government policy is reflected in ex_(11,2) for instance.
...
//  The following lines produce a tauc sequence with a once and for all jump in tauc
// we use shocks to undo that for the first 9 periods and leave tauc at
...
c0=c(1);
k0 = k(1);
// tauc is in ex_(:,2) since it is stored in alphabetical order
tauc0 = ex_(1,2);
...
subplot(2,3,5)
plot([0:9]',ex_(1:10,2),'k','LineWidth',1.5);
hold on;
plot([10:N-1]',ex_(11:N,2),'k','LineWidth',1.5);
hold on;
plot(x,[tauc0*ones(N,1)],'--k','LineWidth',1.5)
axis([0 40 -.1 .5])
title('\tau_c','Fontsize',12)
set(gca,'Fontsize',12)

print -depsc2 fig_tc.eps
